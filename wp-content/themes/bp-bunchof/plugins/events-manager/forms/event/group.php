...
if( !is_super_admin() ){
	foreach( $group_data['groups'] as $group_id ){
                // syp: remove requirement for group admin status for adding event
...
if( count($user_groups) > 0 ){ 
	...
	<select name="group_id">
		<option value=""><?php _e('Select a Group', 'dbem'); ?></option>
		<?php
	...
	<br />
	<em><?php _e ( 'Select a group to schedule an event', 'dbem' )?></em>
	</p>
