 ...
 * cookie_views - sets the name of the cookie to use, it is essentially popup_POST_ID_OF_popup
 * cookie_time - sets the amount of time between views
...
var $close_image = "";
var cookie_views = "popup_cookie_views_" + popup.post_id;
var cookie_time = "popup_cookie_time_" + popup.post_id;
...
jQuery(document).ready(function($){
    ...
    var num_views = (jQuery.cookie(cookie_views));  //gets cookie value for number views
    var next_time_to_show = (jQuery.cookie(cookie_time));  //gets cookie value for number views
    if (!IsNumeric(num_views)) num_views=1;  // tests if cookie value is a number, if not initalizes it
    else num_views++;   //if a number, add one to it
    var current_time = Math.round(+new Date()/1000); // current timestamp
    
    if ((current_time > next_time_to_show) && (number_views === 0 || num_views<=number_views)) {  // if the user has not exhausted # of views for this popup, create it!
        var next_time_to_show = current_time + (parseInt(popup.hours_between_display)*3600);
        jQuery.cookie(cookie_views, num_views, { expires: 365, path: '/' });
        jQuery.cookie(cookie_time, next_time_to_show , { expires: 365, path: '/' });
        prepare_popup();
