...
//function bp_register_upm_widget() {
	add_action('widgets_init', create_function('', 'return register_widget("BP_upm_Widget");') );
//}
//add_action( 'plugins_loaded', 'bp_register_upm_widget' );

class BP_upm_Widget extends WP_Widget {

	function bp_upm_widget() {
   
		parent::WP_Widget( false, $name = __( 'UPM User Location', 'buddypress' ) );
	}
  ...
  */
  ...
  $field = get_option('upm_user_profile_field');
		if ($location = bp_get_profile_field_data( 'field=' . $field . '&user_id=' . bp_displayed_user_id())): ?>
