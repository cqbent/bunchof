  ...
  $style_url = WP_PLUGIN_URL . '/bp-user-profile-map/upm.css';
		$style_file = WP_PLUGIN_DIR . '/bp-user-profile-map/upm.css';
			...
			if (file_exists($style_file)) {
				// If BP parent is enqueueing then add in a dependency handle to array position
				if(function_exists('bp_dtheme_enqueue_styles') ){
				wp_enqueue_style('upm-style', $style_url, array('bp-default-main'), $version, 'all' );
				}else{
...
function upm_setup_globals() {
	global $bp, $wpdb;

	/* For internal identification */
	$bp->upm->id = 'upm';
 
	//$bp->upm->format_notification_function = 'bp_upm_format_notifications';
	//$bp->upm->slug = 'user-location';

	/* Register this in the active components array */
	//$bp->active_components[$bp->upm->slug] = $bp->upm->id;
}

//add_action( 'bp_setup_globals', 'bp_upm_setup_globals' );
 
 /* network_admin_menu */
add_action( 'wp', 'upm_setup_globals', 2 );
if( is_multisite() && function_exists( 'is_network_admin' ) ):
add_action( 'network_admin_menu', 'upm_setup_globals', 2 );
else:
add_action( 'admin_menu', 'upm_setup_globals', 2 );
endif;
...
//bp versions later than 1.2.9 can use bp_core_activation_hook()
// if it workked that is! ? removed as causing issues
//if(function_exists('bp_core_admin_hook') ) {
//	add_action( 'bp_core_admin_hook', 'bp_upm_menu' );
//}else{
	if( is_multisite() && function_exists( 'is_network_admin' ) ):
    	...
    	add_action('bp_before_profile_loop_content', 'upm_gmap_display', 10);
			if( $where == '4.1'){
				...
				function upm_add_map_profile_field(){
					...
					//$grab_xprofile_data	= $field->data->value = bp_unserialize_profile_field( $field->data->value) ;
					if ( bp_get_the_profile_field_name() == $upm_profile_field){
					//$grab_xprofile_data = $grab_xprofile_data . upm_gmap_display();
