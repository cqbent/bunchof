...
function etivite_bp_pending_activations_users() {
	...
	return $wpdb->get_results( "SELECT u.*, m1.meta_value as activation_key FROM $wpdb->usermeta m1, $wpdb->users u WHERE u.ID = m1.user_id AND u.user_status = 2 AND m1.meta_key = 'activation_key' ORDER BY u.user_registered ASC" );
}
	...
	if ( isset( $_POST['submit'] ) && check_admin_referer('etivite_bp_pending_activations_admin') ) {
		...
		if ( isset($_POST['action'] ) && !empty( $_POST['action'] ) && !empty( $_POST['users'] ) ) {
			...
			if ( $_POST['action'] == 'delete' ) {
				...
				$resendusers = $wpdb->get_results( "SELECT m.meta_value, u.ID, u.user_email FROM $wpdb->usermeta m, $wpdb->users u WHERE u.ID = m.user_id AND m.meta_key = 'activation_key' AND u.ID IN ({$userids})" );
				...
				if ( empty( $userids ) )
					wp_die(__( 'No users selected' ) );
				$resendusers = $wpdb->get_results( "SELECT u.ID, u.user_login, m.meta_value FROM $wpdb->usermeta m, $wpdb->users u WHERE u.ID = m.user_id AND m.meta_key = 'activation_key' AND u.ID IN ({$userids})" );
				foreach ( $resendusers as $resend ) {
